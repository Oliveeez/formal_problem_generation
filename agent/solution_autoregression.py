...
class SolutionAutoregressionAgent:
    ...
    async def gen_step_async(
        ...
        """
        # TODO: Warning: state (forward reasoning state) should be re-formatted to solution state (case h.mp and ⊢ False)
        ...
        try:
            ...
            while i_trial < self.max_search_trials:
                ...
                if last_line.startswith('exact') and last_line.endswith('-- submit'):   # Submission
                    ...
                    # Proof search for missing components
    ...
    async def gen_step_async(
        ...
        for _ in range(self.num_samples_per_trial):
            ...
            if not outputs[0].finish_reason == 'stop':
                ...
                continue
            # step = output.message.content.strip()
            # if not (step.startswith('# Next Solution Step\n```lean4\n') and step.endswith('\n```')):
            #     logger.debug(f'gen_steps_async(): Tactic rejected due to wrong format:\n{step}')
            #     continue
            # step = step[len('# Next Solution Step\n```lean4\n'):-len('\n```')].rstrip()
            # if len(re.findall('```', outputs[0].message.content)) == 0:
            #     step = outputs[0].message.content.strip()
            # if len(re.findall('```', outputs[0].message.content)) == 1:
            #     step = outputs[0].message.content[:outputs[0].message.content.find('```')].strip()
            # else:
            #     parse_result = re.findall(CODEBLOCK_PATTERN, outputs[0].message.content)
