...
async def async_worker(
        ...
        data_autoregressive_problem_generation: list
    ):
    ...
    try:
        ...
        data_autoregressive_problem_generation[idx] = {
            "conversation":[
                {
                    "input": f"""
...
""".strip(),
        ...
        logger.warning(f'async_worker({split_base_cnt+idx}): Failed due to {repr(e)}:\n{traceback.format_exc()}')
    finally:
        server.tag = ''
        available_servers.insert(0, server)

async def _async_main(tasks: list, available_servers: list, data_autoregressive_problem_generation: list):
    ...
    for i, (d, u) in tqdm(loop):
        ...
        pending_tasks.add(
            asyncio.create_task(
                async_worker(d, u, i, available_servers, data_autoregressive_problem_generation)
            )
        )
...
def main():
    ...
    data_autoregressive_problem_generation = [None for _ in range(len(tasks))]

    try:
        asyncio.run(_async_main(tasks=tasks, available_servers=available_servers, data_autoregressive_problem_generation=data_autoregressive_problem_generation))
        with open('/home/ma-user/workspace/formal_problem_generation/formal_problem_generation/data/MiniF2F/Numina-Lean/train.deductive_prover.jsonl', 'w') as f:
            for d in data_autoregressive_problem_generation:
                if d is not None:
        ...
        with open('/home/ma-user/workspace/formal_problem_generation/formal_problem_generation/data/MiniF2F/Numina-Lean/train.deductive_prover.pkl', 'wb') as f:
            pickle.dump(data_autoregressive_problem_generation, f)
