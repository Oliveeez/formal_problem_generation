...
async def async_worker(
    ...
    try:
        ...
        remaining_units = [i_u for i_u, u in enumerate(units) if 'deductive_steps' in units[i_u].keys() and 'generation_process' in units[i_u].keys()]
        logger.debug(f'async_worker({base_cnt+idx}): {len(remaining_units)} units to reassemble.')
        ...
        for i_p, i_u in enumerate(remaining_units):
            ...
            try:
                ...
                context, target = decompose_statement(u['formal_statement'])
                ...
                result.formal_statement = new_formal_statement
                result.header = u['load_header']
                result.formal_solution_draft = whole_proof
...
def worker(args: Tuple) -> int:
    ...
    if not osp.exists(osp.join(working_root, f'reassembled_chunk_{base_cnt}.pkl')):
    ...
    with open(osp.join(working_root, f'reassembled_chunk_{base_cnt}.pkl'), 'rb') as f:
