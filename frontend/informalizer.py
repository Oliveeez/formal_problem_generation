...
async def informalize(header: str, formal_statement: str, formal_proof: str, tag_i: int) -> None:
    ...
    client = AsyncOpenAI(
        ...
        api_key='sk-15892db46eb04a93aa681e3459063cc1'   # FPG-Demo
    )
    for i_retry in range(N_RETRY):
        try:
            ...
            return informalization
        except Exception as e:
            logger.debug(f'informalize({tag_i}): {i_retry}-th try failed due to exception {repr(e)}')
            await asyncio.sleep(RETRY_WAIT_TIME)
    logger.opt(colors=True).error(f'informalize({tag_i}): failed')
    return None
