...
'''
    if formal_solution is not None:
        cleaned_formal_solution = '\n'.join([l for l in remove_comments(formal_solution).splitlines() if l.strip() != ''])
        text += f'''# Formal Proof
```lean4
{cleaned_formal_solution}
```
...
def format_icl_prompt(demonstrations: List[Dict], informal_problem: str, forward_context: str) -> str:
    prompt = '''Given an informal math problem and a corresponding Lean 4 goal state, please construct a formal proof deducing the answer.
...
def main(
    ...
    async def search(sample: FormalProblem, tag_i: int) -> None:
        ...
        try:
            ...
            prompt = format_icl_prompt(demonstrations, sample.informal_problem, forward_context)
            n_api_retry = 0
