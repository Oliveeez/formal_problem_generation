...
from common.constants import CORE_OPTIONS, PARSING_EXTRA_OPTIONS
from common.utils import to_sync
from common.pantograph.server import Server, PersistentServer, record_server_error
from common.pantograph.dataclasses import GoalState, TacticDraft
...
-/
def withTypeAscription_ (d : Delab) (cond : Bool := true) : DelabM Term := do
  let stx ← d
  if cond then
...
@[delab mvar]
def delabMVar_ : Delab := do
  let Expr.mvar n ← getExpr | unreachable!
  delabMVarAux_ n

@[delab sort]
def delabSort_ : Delab := do
...
def delabConst_ : Delab := do
  ...
  let stx ← maybeAddBlockImplicit_ stx
  if (← getPPOption getPPTagAppFns) then
    annotateTermInfo stx
  else
    return stx

def withMDataOptions_ [Inhabited α] (x : DelabM α) : DelabM α := do
  match ← getExpr with
  | Expr.mdata m .. =>
...
partial def withMDatasOptions_ [Inhabited α] (x : DelabM α) : DelabM α := do
  if (← getExpr).isMData then withMDataOptions_ (withMDatasOptions_ x) else x

/--
A structure that records details of a function parameter.
-/
structure ParamKind_ where
...
-/
def ParamKind_.isRegularExplicit (param : ParamKind_) : Bool :=
...
-/
def getParamKinds_ (f : Expr) (args : Array Expr) : MetaM (Array ParamKind_) := do
  try
    let mut result : Array ParamKind_ := Array.mkEmpty args.size
...
def shouldShowMotive_ (motive : Expr) (opts : Options) : MetaM Bool := do
...
-/
def unexpandStructureInstance_ (stx : Syntax) : Delab := whenPPOption getPPStructureInstances do
  let env ← getEnv
  let e ← getExpr
  ...
  if hasPPUsingAnonymousConstructorAttribute env s.induct then
    ...
    return ← withTypeAscription_ (cond := (← withType <| getPPOption getPPStructureInstanceType)) do
      `(⟨$[$(stx[1].getArgs)],*⟩)
...
-/
def appFieldNotationCandidate_? : DelabM (Option (Nat × Name)) := do
  let e ← getExpr
...
-/
def needsExplicit_ (f : Expr) (numArgs : Nat) (paramKinds : Array ParamKind_) : Bool :=
...
-/
def delabAppExplicitCore_ (fieldNotation : Bool) (numArgs : Nat) (delabHead : (insertExplicit : Bool) → Delab) (paramKinds : Array ParamKind_) : Delab := do
  let insertExplicit := needsExplicit_ ((← getExpr).getBoundedAppFn numArgs) numArgs paramKinds
  let fieldNotation ← pure (fieldNotation && !insertExplicit) <&&> getPPOption getPPFieldNotation
    <&&> not <$> getPPOption getPPAnalysisNoDot
    <&&> withBoundedAppFn numArgs do pure (← getExpr).consumeMData.isConst <&&> not <$> withMDatasOptions_ (getPPOption getPPAnalysisBlockImplicit <|> getPPOption getPPUniverses)
  let field? ← if fieldNotation then appFieldNotationCandidate_? else pure none
  let (fnStx, _, argStxs) ← withBoundedAppFnArgs numArgs
    ...
    (fun ⟨fnStx, paramKinds, argStxs⟩ => do
      ...
      let argStx ← if some argStxs.size = field?.map Prod.fst then
                     ...
                     withTypeAscription_ (cond := ← getPPOption getPPInstanceTypes) do
...
def AppImplicitArg_.canUnexpand : AppImplicitArg_ → Bool
  | .regular .. | .optional .. | .skip => true
  | .named .. => false

/-- If the argument has associated syntax, returns it. -/
def AppImplicitArg_.syntax? : AppImplicitArg_ → Option Syntax
  | .skip => none
  | .regular s => s
  | .optional _ s => s
  | .named s => s
...
-/
def delabAppImplicitCore_ (unexpand : Bool) (numArgs : Nat) (delabHead : Delab) (paramKinds : Array ParamKind_) : Delab := do
  let unexpand ← pure unexpand
    <&&> withBoundedAppFn numArgs do pure (← getExpr).consumeMData.isConst <&&> not <$> withMDatasOptions_ (getPPOption getPPUniverses)
  let field? ←
    if ← pure unexpand <&&> getPPOption getPPFieldNotation <&&> not <$> getPPOption getPPAnalysisNoDot then
      appFieldNotationCandidate_?
    else
      pure none
  let (fnStx, args') ←
  ...
  if ← pure (unexpand && args'.all (·.canUnexpand)) <&&> getPPOption getPPStructureInstances then
    ...
    if let some stx ← (some <$> unexpandStructureInstance_ stx) <|> pure none then
      return stx
...
where
  mkNamedArg (name : Name) : DelabM AppImplicitArg_ :=
  ...
  -/
  mkArg (param : ParamKind_) : DelabM AppImplicitArg_ := do
    let arg ← getExpr
    if ← getPPOption getPPAnalysisSkip then return .skip
      ...
      -- Assumption: `useAppExplicit_` has already detected whether it is ok to omit this argument, if it is the last one.
    ...
    else if ← pure (param.name == `motive) <&&> shouldShowMotive_ arg (← getOptions) then
      mkNamedArg param.name
    else
      return .skip
  /--
  ...
  -/
  tryAppUnexpanders (fnStx : Term) (args : Array AppImplicitArg_) : Delab := do
...
-/
def useAppExplicit_ (numArgs : Nat) (paramKinds : Array ParamKind_) : DelabM Bool := do
  ...
  if ← withBoundedAppFn numArgs <| withMDatasOptions_ <| getPPOption getPPAnalysisBlockImplicit then
    return true
...
* If `unexpand` is true, then allow unexpanders and field notation.
  This should likely be set to `false` except in the main `delabApp_` delaborator.

Propagates `pp.tagAppFns` into the head for `delabHead`.
-/
def delabAppCore_ (maxArgs : Nat) (delabHead : (insertExplicit : Bool) → Delab) (unexpand : Bool) : Delab := withTypeAscription_ (cond := true) do
  ...
  let paramKinds ← getParamKinds_ fn args
  if (← useAppExplicit_ args.size paramKinds) then
    -- Don't use field notation when `pp.tagAppFns` is true since that obscures the head application.
    delabAppExplicitCore_ (fieldNotation := unexpand && !tagAppFn) args.size delabHead' paramKinds
  else
    delabAppImplicitCore_ (unexpand := unexpand) args.size (delabHead' false) paramKinds

/--
Default delaborator for applications.
-/
@[delab app]
def delabApp_ : Delab := do
  let delabAppFn (insertExplicit : Bool) : Delab := do
    let stx ← if (← getExpr).consumeMData.isConst then withMDatasOptions_ delabConst_ else delab
    if insertExplicit && !stx.raw.isOfKind ``Lean.Parser.Term.explicit then `(@$stx) else pure stx
  delabAppCore_ (← getExpr).getAppNumArgs delabAppFn (unexpand := true)

/--
The `withOverApp_` combinator allows delaborators to handle "over-application" by using the core
...
-/
def withOverApp_ (arity : Nat) (x : Delab) : Delab := do
  ...
  if n == arity then
    ...
    delabAppCore_ (n - arity) delabHead (unexpand := false)

@[delab app]
def delabDelayedAssignedMVar_ : Delab := whenNotPPOption getPPMVarsDelayed do
  let .mvar mvarId := (← getExpr).getAppFn | failure
  let some decl ← getDelayedMVarAssignment? mvarId | failure
  withOverApp_ decl.fvars.size do
    ...
    delabMVarAux_ decl.mvarIdPending

/-- State for `delabAppMatch_` and helpers. -/
structure AppMatchState_ where
  info        : MatcherInfo
  ...
  Once `AppMatchState_` is complete, this is not `none`. -/
...
-/
@[delab app]
partial def delabAppMatch_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation <| whenPPOption getPPMatch do
  ...
  withOverApp_ info.arity do
    -- First pass visiting the match application. Incrementally fills `AppMatchState_`,
    -- collecting information needed to delaborate the patterns and RHSs.
    -- No need to visit the parameters themselves.
    let st : AppMatchState_ ← withBoundedAppFnArgs (1 + info.numDiscrs + info.numAlts)
      (do
    ...
    if st.rhss.isEmpty then
      ...
      if ← pure st.motiveNamed <||> shouldShowMotive_ lamMotive opts then
...
-/
@[delab app.letFun]
def delabLetFun_ : Delab := whenPPOption getPPNotation <| withOverApp_ 4 do
  let e ← getExpr
...
@[delab app.Char.ofNat]
def delabChar_ : Delab := do
  let e ← getExpr
...
-/
def delabOfNatCore_ (showType : Bool := false) : Delab := do
...
-/
def delabNegIntCore_ (showType : Bool := false) : Delab := do
...
-/
def delabDivRatCore_ (showType : Bool := false) : Delab := do
  let e ← getExpr
...
def delabOfNat_ : Delab := do
  let showType ← getPPOption getPPNumericTypes
  whenNotPPOption getPPExplicit <| whenPPOption getPPCoercions <| withOverApp_ 3 do
...
-/
@[delab app.Neg.neg]
def delabNeg_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPCoercions do
...
-/
@[delab app.HDiv.hDiv]
def delabHDiv_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPCoercions do
...
def delabOfScientific_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPCoercions <| withOverApp_ 5 do
  let expr ← getExpr
...
-/
@[delab app]
def coeDelaborator_ : Delab := whenPPOption getPPCoercions do
  let e ← getExpr
  ...
  if n == info.numArgs then
    ...
    delabAppCore_ nargs (delabHead info nargs) (unexpand := false)
where
  delabHead (info : CoeFnInfo) (nargs : Nat) (insertExplicit : Bool) : Delab := do
    withTypeAscription_ (cond := ← getPPOption getPPCoercionsTypes) do
      guard <| !insertExplicit
...
@[delab app.dite]
def delabDIte_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation <| withOverApp_ 5 do
...
@[delab app.cond]
def delabCond_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation <| withOverApp_ 4 do
...
def delabNamedPattern_ : Delab := do
...
def delabSigmaCore_ (sigma : Bool) : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation do
...
@[delab app.Sigma]
def delabSigma_ : Delab := delabSigmaCore_ (sigma := true)

@[delab app.PSigma]
def delabPSigma_ : Delab := delabSigmaCore_ (sigma := false)

-- PProd and MProd value delaborator
-- (like pp_using_anonymous_constructor but flattening nested tuples)

def delabPProdMkCore_ (mkName : Name) : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation do
  guard <| (← getExpr).getAppNumArgs == 4
...
@[delab app.PProd.mk]
def delabPProdMk_ : Delab := delabPProdMkCore_ ``PProd.mk

@[delab app.MProd.mk]
def delabMProdMk_ : Delab := delabPProdMkCore_ ``MProd.mk

partial def delabDoElems_ : DelabM (List Syntax) := do
  let e ← getExpr
  ...
  where
    prependAndRec x : DelabM _ := List.cons <$> x <*> delabDoElems_

@[delab app.Bind.bind]
def delabDo_ : Delab := whenNotPPOption getPPExplicit <| whenPPOption getPPNotation do
  guard <| (← getExpr).isAppOfArity ``Bind.bind 6
  let elems ← delabDoElems_
  let items ← elems.toArray.mapM (`(doSeqItem|$(·):doElem))
  `(do $items:doSeqItem*)

/-- Delaborates a function application of the form `f ... x (fun _ : Unit => y)`. -/
def delabLazyBinop_ (arity : Nat) (k : Term → Term → DelabM Term) : DelabM Term :=
  whenNotPPOption getPPExplicit <| whenPPOption getPPNotation <| withOverApp_ arity do
    let y ← withAppArg do
...
@[delab app.HOrElse.hOrElse]
def delabHOrElse_ : Delab := delabLazyBinop 6 (fun x y => `($x <|> $y))

@[delab app.HAndThen.hAndThen]
def delabHAndThen_ : Delab := delabLazyBinop 6 (fun x y => `($x >> $y))

@[delab app.Seq.seq]
def delabSeq_ : Delab := delabLazyBinop 6 (fun x y => `($x <*> $y))

@[delab app.SeqLeft.seqLeft]
def delabSeqLeft_ : Delab := delabLazyBinop 6 (fun x y => `($x <* $y))

@[delab app.SeqRight.seqRight]
def delabSeqRight_ : Delab := delabLazyBinop 6 (fun x y => `($x *> $y))

@[delab app.Lean.Name.str,
  delab app.Lean.Name.mkStr1, delab app.Lean.Name.mkStr2, delab app.Lean.Name.mkStr3, delab app.Lean.Name.mkStr4,
  delab app.Lean.Name.mkStr5, delab app.Lean.Name.mkStr6, delab app.Lean.Name.mkStr7, delab app.Lean.Name.mkStr8]
def delabNameMkStr_ : Delab := whenPPOption getPPNotation do
...
@[delab app.Lean.Name.num]
def delabNameMkNum_ : Delab := delabNameMkStr

open Parser Command Term in
@[run_builtin_parser_attribute_hooks]
-- use `termParser` instead of `declId` so we can reuse `delabConst_`
def declSigWithId_ := leading_parser termParser maxPrec >> declSig
...
-/
partial def delabConstWithSignature_ (universes : Bool := true) : Delab := do
  let e ← getExpr
...
class PersistentParsingServer(PersistentServer):
    def __init__(self, max_count: int=64, is_state_based: bool=True, tag: str='', _sync_init: bool=True, *args, **kwargs):
        ...
        # Set-up parsing server
        if 'core_options' in self.server_kwargs.keys():
            assert self.server_kwargs['core_options'] == CORE_OPTIONS, 'Core options of parsing server is fixed'
        self.server_kwargs['imports'] = PARSING_IMPORTS + self.server_kwargs.get('imports', [])
        self.server_kwargs['core_options'] = CORE_OPTIONS + PARSING_EXTRA_OPTIONS
    ...
    async def check_restart_async(self) -> None:
        ...
        if self.is_state_based:
            units = await self.server.load_sorry_async(PARSING_PRELUDE + '''example : True := sorry''')
    ...
    async def load_statement_async(self, statement: str, intros: List[str]=[], header: str='') -> GoalState:
        ...
        init_state = await self.server.goal_tactic_async(
