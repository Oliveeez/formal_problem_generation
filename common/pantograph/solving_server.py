...
from common.constants import CORE_OPTIONS, PARSING_EXTRA_OPTIONS, OPEN_HEADER, IDENT_MAP, RPE_TACTICS, H_SUBMISSION_NAME, FALSIFY_TACTICS, BANNED_TOKENS_IN_SOLVING_STATE, BANNED_TOKENS_IN_ANSWER_TYPE
from common.utils import (
    format_variable_sequence, inplace_add,
    parse_idents,
    replace_calc, replace_sorry,
    remove_comments, remove_spaces, normalize_spaces,
    solution_decompose, flip_options,
    split_list, chunk_list,
    to_sync)
...
class VacuousProblemError(Exception):
    pass
...
class BaseSolvingServer:
    ...
    async def load_problem_async(self, sample: Optional[Union[FormalProblem, DataPoint, str]], force_parse: bool=False) -> None:
        ...
        if isinstance(sample, FormalProblem):
            ...
            # DataPoint is a deprecated dataclass used in `numina_cot_qwen2.verified.valid.cleaned-cycle1-deepseek_v3.pkl`.
        ...
        # option_header = '\n'.join(f'set_option ' + o.replace('=', ' ') + ' in' for o in PARSING_EXTRA_OPTIONS)    # ['pp.numericTypes=true', 'pp.structureInstances=false', 'pp.safeShadowing=false', 'pp.fieldNotation.generalized=false', 'pp.explicit=true', 'pp.deepTerms=true']
        ...
        # init_falsify_state = await self.server.goal_tactic_async(init_states[-1].goal_state, 0, 'exfalso')
        # assert init_falsify_state.goals[0].target == 'False'
        # for tac in FALSIFY_TACTICS:
        #     try:
        #         falsify_state = await self.server.goal_tactic_async(init_falsify_state, 0, tac)
        #     except:
        #         continue
        #     if falsify_state.is_solved:
        #         raise UnsatisfiableProblemError(f'Falsify tactic: {tac}')
        ...
        #     if remove_spaces(self.sample.formal_answer) in remove_spaces(str(v)):
        #         raise VacuousProblemError(normalize_spaces(remove_comments(sample.formal_statement)))
...
class TermSolvingServer(BaseSolvingServer):
    ...
    async def init_solving_state_async(self, sample: Optional[Union[FormalProblem, DataPoint, str]]=None) -> Tuple[GoalState, List[str]]:
        await self.load_problem_async(sample)
        tac_history = []
...
class PersistentBaseSolvingServer:
    def __init__(
        self,
        imports: List[str]=["Mathlib", "Aesop"],
        project_path: Optional[str]=None,
        ...
        self.sample: Optional[FormalProblem] = None
        self.answer_mvarId: Optional[str] = None
        self.formal_problem_framework: Optional[str] = None
        # Setting
        self.max_count = max_count
        self.tag = tag
        # Persistent Server
        self.load_server = PersistentServer(
            max_count=max_count,
            is_state_based=False,
            tag=tag+'/load_server',
            _sync_init=_sync_init,
            imports=imports,
            project_path=project_path,
            core_options=CORE_OPTIONS + PARSING_EXTRA_OPTIONS,
            timeout=timeout,
        )
        self.tactic_server = PersistentServer(
            max_count=max_count,
            is_state_based=True,
            tag=tag+'/tactic_server',
            _sync_init=_sync_init,
            imports=imports,
            project_path=project_path,
            core_options=CORE_OPTIONS,
    ...
    def set_tag(self, tag) -> None:
        self.tag = tag
        self.load_server.tag = tag+'/load_server'
        self.tactic_server.tag = tag+'/tactic_server'

    async def load_problem_async(
        self,
        sample: Optional[Union[FormalProblem, DataPoint, str]]=None,
        ...
        # Load problem
        if isinstance(sample, FormalProblem):
            if not force_parse and sample.outros is not None and len(sample.outros) > 0:
                # Assume problems in `FormalProblem` style pass all checks, such as type check, falsify check, etc.
                self.sample = sample
                return sample
        elif isinstance(sample, DataPoint):
        ...
        # Parse Problem
        formal_statement = remove_comments((sample.header or OPEN_HEADER)+'\n'+sample.formal_statement)
        init_state = await self.load_server.load_example_async(formal_statement) # sample.header is added to help parse existing data
        
        init_state_str = str(init_state)
        assert H_SUBMISSION_NAME not in init_state_str, f'load_problem_async({self.tag}): Statement conflict with H_SUBMISSION_NAME "{H_SUBMISSION_NAME}" in {[init_state_str]}'
        for t in BANNED_TOKENS_IN_SOLVING_STATE:
            assert t not in init_state_str, f'load_problem_async({self.tag}): Detected banned token "{t}" in {[init_state_str]}'
        
        formal_problem_parsed = init_state.goals[0]
        formal_answer_components = set(parse_idents(formal_problem_parsed.target))
        
        # Vacuous check
        sample.formal_answer = formal_problem_parsed.target
        for v in formal_problem_parsed.variables:
            if remove_spaces(sample.formal_answer) in remove_spaces(str(v)):
                raise VacuousProblemError(normalize_spaces(remove_comments(sample.formal_statement)))
            
        if 'answer' not in remove_comments(sample.formal_answer).strip():
            logger.warning(f'load_problem_async({self.tag}): Invalid formal answer: {remove_comments(sample.formal_answer).strip()}, running in unsupervised mode.')
            sample.formal_answer = None
        ...
        named_vartypes = dict()
        anonymous_vartypes = []
        for line in parse_variables(remove_comments(sample.formal_statement)):
            if not line.startswith('['):
                line = line[1:-1]
                split_pos = line.find(':')
                assert split_pos != -1, f'load_problem_async({self.tag}): Unable to find variable names in `{line}`'
        ...
        for v in formal_problem_parsed.variables:
            ...
            if v.name == 'answer':
                assert outros is None
                outros = []
                sample.formal_answer_type = v.t
                for t in BANNED_TOKENS_IN_ANSWER_TYPE:
                    assert t not in sample.formal_answer_type, f'load_problem_async({self.tag}): Detected banned token "{t}" in answer type "{sample.formal_answer_type}"'
                continue
            else:
                if outros is None:
                    intros.append(v)
        ...
        sample.intros = intros
        sample.outros = outros

        # Falsify verification
        formal_proposition = '∀ ' + \
            ((format_variable_sequence(sample.intros) + ' ') if len(sample.intros) > 0 else '') + \
            f'(answer : {sample.formal_answer_type}) ({H_SUBMISSION_NAME}: {sample.formal_answer}) ' + \
            ((format_variable_sequence(sample.outros) + ' ') if len(sample.outros) > 0 else '') + \
            f', False'
        init_falsify_state = await self.tactic_server.load_statement_async(
            statement=formal_proposition,
            intros=[v.name or '_' for v in sample.intros] + ['answer', H_SUBMISSION_NAME] + [v.name or '_' for v in sample.outros],
            header=sample.header or OPEN_HEADER,
        )
        assert init_falsify_state.goals[0].target == 'False'
        ...
        try:
            for tac in FALSIFY_TACTICS:
                try:
                    falsify_state = await self.tactic_server.goal_tactic_async(init_falsify_state, 0, tac)
                except TacticFailure:
                    continue
                if falsify_state.is_solved:
                    raise UnsatisfiableProblemError(f'Falsify tactic: {tac}')
        except Exception as e:
            if isinstance(e, UnsatisfiableProblemError):
                raise e
            else:
                pass
    ...
    async def get_submission_async(self, state: GoalState) -> Optional[str]:
        assert self.answer_mvarId is not None
        rs = await self.tactic_server.goal_print_async(state, False, False, False, [self.answer_mvarId])
        answer = rs['extraMVars'][0]
        return answer.get('pp', None)


class PersistentPropSolvingServer(PersistentBaseSolvingServer):
    # Load from a Datapoint or statement, returning current solution state and solution goals to ignore
    async def init_solving_state_async(self, sample: Optional[Union[FormalProblem, DataPoint, str]]=None) -> GoalState:
        await self.load_problem_async(sample)
        ...
        init_solution_state = await self.tactic_server.load_statement_async(
            statement=self.formal_problem_framework,
            header=(self.sample.header or OPEN_HEADER),
            intros=[v.name or '_' for v in self.sample.intros] + ['answer'])
        
        init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 0, 'apply Exists.intro')
        
        rs = await self.tactic_server.goal_print_async(init_solution_state, False, False, True)
        answer_mvarId = [g['name'] for g in rs['goals'] if g['userName'] == 'w']
        assert len(answer_mvarId) == 1
        self.answer_mvarId = answer_mvarId[0]
        
        if len(self.sample.outros) > 1:
            init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 0, 'intros ' + ' '.join([v.name or '_' for v in self.sample.outros[:-1]]))
        init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 0, 'constructor')
        init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 1, f'intros {H_SUBMISSION_NAME}')
        init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 1, 'intros ' + self.sample.outros[-1].name)
    ...
    async def init_forward_solving_state_async(self, sample: Optional[Union[FormalProblem, DataPoint, str]]=None) -> GoalState:
        ...
        if len(self.sample.outros) > 0:
            init_solution_state = await self.tactic_server.goal_tactic_async(init_solution_state, 0, 'intros ' + ' '.join([v.name or '_' for v in self.sample.outros]))
    ...
    async def init_forward_reasoning_state_async(self, sample: Optional[Union[FormalProblem, DataPoint, str]]=None) -> GoalState:
        ...
        init_solution_state = await self.tactic_server.load_statement_async(
            statement=self.formal_problem_framework,
            header=(self.sample.header or OPEN_HEADER),
            intros=[v.name or '_' for v in self.sample.intros] + ['answer'] + [v.name or '_' for v in self.sample.outros])
    ...
    async def prove_eq_async(self, submission: str) -> Optional[str]:
        ...
        init_state = await self.tactic_server.load_statement_async(
            statement=rpe_code,
            header=(self.sample.header or OPEN_HEADER),
            intros=[v.name or '_' for v in self.sample.intros] + ['answer'])
        ...
        try:
            for tac in RPE_TACTICS:
                try:
                    state = await self.tactic_server.goal_tactic_async(init_state, 0, tac)
                    if state.is_solved:
                        return tac
                except TacticFailure:
                    pass
