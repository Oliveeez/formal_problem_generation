...
for (idx, sample) in enumerate(samples):
        ...
        for i_step, draft_step in enumerate(solution_blocks[:-1]):
            ...
            for v in forward_state.goals[0].variables:
                ...
                if '✝' not in v_to_remove.name:
                    try:
                        new_tmp_parsing_state = server.tactic_server.goal_tactic(tmp_parsing_state, 0, f'clear! {v_to_remove.name}')
                    except TacticFailure as e:
                        soft_dependencies.add(v_to_remove.raw_name)
                        logger.warning(f'async_worker({0+idx}): Cannot remove {v_to_remove} ({[vv.name for vv in parsed_steps[fvarid_to_istep[v_to_remove.raw_name]].new_contexts]})')
                        continue
                else:
                ...
                try:
                    ...
                    # logger.debug(f'async_worker({0+idx}): {[vv.name for vv in cur_step.new_contexts]} depends on {[vv.name for vv in parsed_steps[fvarid_to_istep[v_to_remove.raw_name]].new_contexts]}')
                    continue
        ...
        with open(f'./reassembled_trajectory_{idx}.txt', 'w') as f:
